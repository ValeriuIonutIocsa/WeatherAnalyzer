	...
	static void work(
		...
		try {
			...
			if (IoUtils.directoryExists(PathUtils.createRootPath())) {
				outputFilePathString = PathUtils.computePath(PathUtils.createRootPath(),
			...
			FactoryFolderCreator.getInstance().createParentDirectories(outputFilePathString, true);
			...
			try (final PrintStream printStream = StreamUtils.openPrintStream(outputFilePathString)) {
